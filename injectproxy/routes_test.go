...
func TestWithPassthroughPaths(t *testing.T) {
	...
	t.Run("invalid passthrough options", func(t *testing.T) {
		// Duplicated /api.
		_, err := NewRoutes(m.url, proxyLabel,adminGroup,defaultGroup,config_ch, WithPassthroughPaths([]string{"/api1", "/api2/something", "/api1"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// Wrong format, params in path.
		_, err = NewRoutes(m.url, proxyLabel,adminGroup,defaultGroup,config_ch,WithPassthroughPaths([]string{"/api1?args=1", "/api1"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// / is not allowed.
		_, err = NewRoutes(m.url, proxyLabel,adminGroup,defaultGroup,config_ch,WithPassthroughPaths([]string{"/", "/api2/something", "/api1"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// "" is not allowed.
		_, err = NewRoutes(m.url, proxyLabel,adminGroup,defaultGroup,config_ch, WithPassthroughPaths([]string{"/api1", "/api2/something", "", "/api3"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// Duplication with existing enforced path is not allowed.
		_, err = NewRoutes(m.url, proxyLabel,adminGroup,defaultGroup,config_ch, WithPassthroughPaths([]string{"/api1", "/api2/something", "/federate", "/api3"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// Duplication with existing enforced path is not allowed.
		...
		// Duplication with existing enforced path is not allowed.
		_, err = NewRoutes(m.url, proxyLabel, adminGroup,defaultGroup,config_ch, WithPassthroughPaths([]string{"/api1", "/api2/something", "/federate/some", "/api3"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// api4 is not valid URL path (does not start with /)
		_, err = NewRoutes(m.url, proxyLabel, adminGroup, defaultGroup, config_ch, WithPassthroughPaths([]string{"/api1", "/api2/something", "api4", "/api3"}))
		if err == nil {
			t.Fatal("expected error")
		}
		// api4/ is not valid URL path (does not start with /)
		...
		_, err = NewRoutes(m.url, proxyLabel, adminGroup, defaultGroup, config_ch, WithPassthroughPaths([]string{"/api1", "/api2/something", "api4/something", "/api3"}))
		if err == nil {
			t.Fatal("expected error")
		}
	})
	r, err := NewRoutes(m.url, proxyLabel, adminGroup, defaultGroup, config_ch, WithPassthroughPaths([]string{"/api1", "/api2/something", "/graph/"}))
	if err != nil {
	...
	for _, tc := range []struct {
		...
		for _, u := range []string{
			...
			t.Run(fmt.Sprintf("%s?match[]=%s", u, strings.Join(tc.matches, "&")), func(t *testing.T) {
				...
				defer m.Close()
				r, err := NewRoutes(m.url, proxyLabel,adminGroup, defaultGroup, config_ch,WithEnabledLabelsAPI())
				if err != nil {
					t.Fatalf("unexpected error: %v", err)
				}
	...
	for _, tc := range []struct {
		...
		for _, endpoint := range []string{"query", "query_range", "query_exemplars"} {
			t.Run(endpoint+"/"+strings.ReplaceAll(tc.name, " ", "_"), func(t *testing.T) {
				...
				r, err := NewRoutes(m.url, proxyLabel,adminGroup, defaultGroup, config_ch,opts...)
				if err != nil {
					t.Fatalf("unexpected error: %v", err)
				}
